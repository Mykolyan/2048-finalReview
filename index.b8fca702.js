...
function displayBoard() {
    ...
    cells.forEach((cell, index)=>{
        ...
        const currentClass = Array.from(cell.classList).find(function(className) {
            return className.startsWith("field-cell--") && className !== "field-cell--spawn" && className !== "field-cell--merge";
        });
        ...
        if (cellValue !== 0 && cellValue !== prevValue) {
            ...
            cell.offsetWidth;
            cell.classList.add(prevValue === 0 ? "field-cell--spawn" : "field-cell--merge");
        }
    });
...
document.querySelector(".start, .restart").addEventListener("click", (e)=>{
    ...
    if (button.classList.contains("start")) startGame();
    else if (button.classList.contains("restart")) restartGame();
});
